    ...
    return (
        <>
            ...
            <li className={`list-group-item ${task.priority}`}>
                ...
                {isUpdate?<input type="text" className="form-control"  placeholder={task.title} id={`edit_${task.id}`} onChange={(e)=>setValue(e.target.value)}/>:<label className="form-check-label" htmlFor={`check_${task.id}`}>{task.title}</label>}
